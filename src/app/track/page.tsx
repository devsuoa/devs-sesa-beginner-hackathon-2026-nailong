...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className="inline-block px-3 py-1 text-[9px] font-black tracking-[0.2em]"
      style={{
  ...
  return (
    <div className="relative h-28 overflow-hidden">
      ...
      <div className="absolute top-1/2 left-4 h-px -translate-y-1/2 transition-all duration-1000"
        style={{ width: `calc(${progress}% - 16px)`, background: "rgba(100,255,180,0.3)" }} />
        ...
        <svg width="40" height="24" viewBox="0 0 40 24" fill="none">
          ...
          <path d="M5 10 L-6 12 L5 14 Z" fill="rgba(100,255,180,0.6)">
            <animate attributeName="d"
              values="M5 10 L-6 12 L5 14 Z;M5 9 L-9 12 L5 15 Z;M5 10 L-6 12 L5 14 Z"
...
function TrackingContent() {
  ...
  const { loading: authLoading } = useAuthGuard("RIDER");
  ...
  async function fetchBooking() {
    try {
      ...
      if (!res.ok) {
        ...
        setFetchError(data.error ?? "Booking not found.");
        return;
      }
      const data = await res.json();
  ...
  async function handleCancel() {
    ...
    try {
      ...
      if (res.ok) { await fetchBooking(); setShowCancelModal(false); }
    } finally {
      setCancelling(false);
    }
  }
  ...
  const isActive = booking && ["PENDING", "CONFIRMED", "IN_TRANSIT"].includes(booking.status);
  const isCancelled = booking?.status === "CANCELLED";
  const isDelivered = booking?.status === "DELIVERED";

  return (
    <AuthGate loading={authLoading}>
      {fetchLoading ? (
        ...
        <div className="min-h-screen flex flex-col items-center justify-center gap-4">
          <p className="text-[10px] tracking-[0.3em] text-red-400/70"
            style={{ fontFamily: "'Share Tech Mono', monospace" }}>
            !! {(fetchError ?? "BOOKING NOT FOUND").toUpperCase()}
          </p>
          <Link href="/ride"
            className="text-[10px] tracking-[0.2em] text-white/35 hover:text-white transition-colors"
        ...
        <div className="w-full max-w-xl mx-auto px-6 py-10 mt-16">
          ...
          <Card title="route">
            ...
            {booking.destination.currentConditions && (
              <Row label="CONDITIONS" value={booking.destination.currentConditions} />
            )}
          ...
          <div className="flex gap-3 mt-2">
            ...
            {isActive && (
              <button type="button" onClick={() => setShowCancelModal(true)}
                className="flex-1 relative flex items-center justify-center py-3 text-[10px] font-black tracking-[0.2em] transition-all bg-transparent border-none cursor-pointer"
                style={{
                  fontFamily: "'Orbitron', monospace",
              ...
              <div className="relative w-full max-w-sm p-6"
                ...
                <div className="flex gap-3">
                  <button type="button" onClick={() => setShowCancelModal(false)}
                    className="flex-1 relative py-2.5 text-[10px] font-black tracking-[0.2em] text-white/50 hover:text-white transition-all bg-transparent border-none cursor-pointer"
                  ...
                  <button type="button" onClick={handleCancel} disabled={cancelling}
                    className="flex-1 relative py-2.5 text-[10px] font-black tracking-[0.2em] transition-all bg-transparent border-none disabled:opacity-40 cursor-pointer"
          ...
          {showRating && (
            <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
              ...
              <div className="relative w-full max-w-sm p-8 text-center"
                ...
                <button type="button"
                  onClick={() => { setShowRating(false); router.push("/orders"); }}
